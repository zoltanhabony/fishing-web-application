...
export async function editCatch(
  ...
  if(!currentUser){
    return {
        errors: {
          ...
          status: "error",
          description: "There is no user with this session! Sign in again! ",
        },
      };
  }
  ...
  if (session.user.role !== UserRole.USER) {
    return {
      errors: {
        _form: ["Operation failed"],
        subtitle: "No access to perform the operation",
        status: "error",
        description: "You do not have appropriate role to create catch!",
      },
    };
  }

  if(!currentUser.access[0].accessToLogbook && !currentUser.access[0].accessToFishing){
    return {
        errors: {
          _form: ["Operation failed"],
          subtitle: "No access to perform the operation",
          status: "error",
          description: "You do not have permission to create catch!",
        },
      };
  }
  ...
  if(result.data.length === 0) {
    return {
      errors: {
        weight: ["The fish size must be given!"],
        _form: ["Unsuccessful data saving!"],
  ...
  if(result.data.piece === 0) {
    return {
      errors: {
        weight: ["The fish piece must be given!"],
        _form: ["Unsuccessful data saving!"],

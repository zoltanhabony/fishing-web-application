...
export async function createCatch(
  ...
  if (!isFishing || !access?.accessToLogbook) {
    return {
      errors: {
        ...
        status: "error",
        description:
          "You are not qualified to recognize catch, contact the competent authority for more information!",
      },
    };
  }
  ...
  if (rule) {
    ...
    if (result.data.isStored) {
      if (constraint && constraint.banPeriodStart && constraint.banPeriodEnd) {
        if (
          currentDate >= constraint.banPeriodStart &&
          currentDate <= constraint.banPeriodEnd
        ) {
          if (
            !constraint.maxLengthLimit &&
            !constraint.minLengthLimit &&
            !constraint.maxWeightLimit &&
            !constraint.minWeightLimit &&
            !constraint.pieceLimit
          ) {
            return {
              errors: {
                ...
                status: "error",
                description:
                  "The fish is currently under a closed season in the specified water area or under a national water order, the fish cannot be taken home but can be recorded!",
              },
            };
          }
          ...
          if (
            constraint.minLengthLimit &&
            Number(constraint.minLengthLimit) > result.data.length
          ) {
            return {
              errors: {
                fishId: ["The fish is currently under a closed season!"],
          ...
          if (
            constraint.maxLengthLimit &&
            Number(constraint.maxLengthLimit) < result.data.length
          ) {
            return {
              errors: {
                fishId: ["The fish is currently under a closed season!"],
          ...
          if (
            constraint.minWeightLimit &&
            Number(constraint.minWeightLimit) > result.data.weight
          ) {
            return {
              errors: {
                fishId: ["The fish is currently under a closed season!"],
          ...
          if (
            constraint.maxWeightLimit &&
            Number(constraint.maxWeightLimit) < result.data.weight
          ) {
            return {
              errors: {
                fishId: ["The fish is currently under a closed season!"],
          ...
          if (
            constraint.pieceLimit &&
            constraint.pieceLimit < result.data.piece + piceOfFishType._count.id
          ) {
            return {
              errors: {
                fishId: ["The fish is currently under a closed season!"],
                _form: ["Unsuccessful data saving!"],
                subtitle:
                  "You have reached the maximum number of fish that can be caught!",
                status: "error",
                description:
                  "The fish is currently under a closed season in the specified water area or under a national water order, the fish cannot be taken home but can be recorded!",
              },
  ...
  try {
    ...
    return {
      errors: {
        ...
        status: "success",
        description:
          "The fish saved successfully! You can view the saved fish at catches.",
      },
    };
  } catch (error: any) {
    return {

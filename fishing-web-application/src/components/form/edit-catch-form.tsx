...
export const EditCatchForm = (data: EditLogbookFormProps) => {
  const [formState, action] = useFormState(actions.editCatch, {
    errors: {},
  });
      ...
      <form action={action} className="space-y-3 w-full pt-3">
        ...
        <Input
          ...
          <div className="w-full space-y-4 sm:flex sm:space-x-3 sm:space-y-0">
            <Input
              defaultValue={data.data?.CatchDetails[0].value !== null ? String(data.data?.CatchDetails[0].value) : undefined}
              name="weight"
              label="Weight"
              type="number"
              variant="bordered"

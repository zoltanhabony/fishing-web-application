...
export const ApplyTournamentForm = ({ id }: ApplyTournamentForm) => {
  const [formState, action] = useFormState(actions.applyForTournament, {
    errors: {},
  });
  ...
  return (
    <form action={action} className="space-y-3 w-full pt-3">
      <div className="flex flex-col gap-4 w-full">
        ...
        {session.data?.user.role === "USER" || session.data?.user.role === "INSPECTOR" ? <Button color="primary" fullWidth type="submit" className="mt-3">
          Apply for
        </Button>: ""}
      </div>
    </form>
